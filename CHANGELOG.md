...
### 📢 被 @ 提及 · 必达提醒
- 消息里输入 `@名字` / `@ID` 提及成员：对方收到**必达提醒**——三音上行提示音 + 页面可见也弹桌面通知 + 通知中心「@我」打标
- 发送端确认提示：发完自动提示「已提及 xx」，对方会收到提醒
- 纯前端实现，@ 不改变消息文本，任意成员都可使用

### 👥 @ 提及成员浮层
- 消息输入框新增「@」按钮：弹出全员成员浮层（姓名 + ID），点击自动插入 `@名字 `
- 普通成员也能看到全员概要（来自全员可读的成员字段）

### 🎯 仪表盘「我的任务」
- 仪表盘新增「我的任务」面板：我负责的任务卡片（状态/优先级/逾期/🔔），一键进详情
- 待办计数徽章；任务板新增「🙋 只看我的」筛选

### 质量
- 面板 JS 语法校验通过；单测 56 项全绿；冒烟 177 项全绿

## [0.24.0] — 2026-09-16

### 🗣 消息语音朗读（微信式语音播报）
- 每条消息「🔊 朗读」：Web Speech API 中文朗读（零依赖），消息页顶栏/设置页可开关
- 开启后**新消息自动语音播报**（他人消息，自己发送的回声不读），微信同款体验

### 📄 团队报告导出（CSV）
- 仪表盘 / 设置页 / 命令面板一键导出 `coagent-report-日期.csv`：成员贡献榜、任务状态分布、14 天趋势（事件/当日完成/累计）、上下文与文件类型分布
- 中文 UTF-8 BOM，Excel 直接打开不乱码

### 📋 任务详情弹窗升级
- 底部新增「🔕 关注任务」快捷按钮（与任务卡 🔔 联动）
- 未认领任务可直接「🤝 认领」，自己认领的进行中任务可直接「✓ 完成」——详情内完成状态流转，无需退回任务板
...
### 🟢 成员在线拓扑（新页面）
- 实时卡片网格：每台电脑的 agent 一张卡——人格头像 / 姓名 / ID / 在线呼吸点 / 最近活跃 / 本周动作数
- 数据来自 WebSocket 心跳（`/stats` 新增全员可读的成员概要，不含 token/scopes 等敏感字段）

### 📈 效率分析页（新页面）
- 成员贡献榜（近 7 天，倒序条形）
- 每人动作构成：任务 / 上下文 / 评论 / 审核 / 分支 / 消息 / 文件 类型分布色板
- 近 14 天活动趋势 + 任务状态环形图

### 🔔 任务关注机制
- 任务卡新增 🔔 关注按钮（localStorage 持久化）
- 关注的任务：变更**必达**——通知中心打「📌 关注」标 + 页面可见时也弹桌面通知 + **三音上行必达提示音**
- 任务板「只看关注」筛选，一键聚焦自己盯的任务
- 成员在线卡 📈 直达效率分析并聚焦该成员（🎯 聚焦横幅 + 清除）

### 🎨 皮肤库 3 → 5 套
- 新增「午夜紫」（深紫夜航 · 紫粉渐变）与「暖纸极简」（米白纸张 · 砖橙主色）

### 🚀 命令面板
- 新增「成员在线」「效率分析」快捷命令

### 质量
- 单测 56 项全绿；冒烟 177 项全绿（新增成员概要结构与敏感字段隔离断言）

## [0.22.0] — 2026-09-16

### 🧠 人格库扩充至 14 种 + 人格库浏览页
- 新增 6 种人格：视觉设计师（🎨 体验/视觉一致）、运维哨兵（🛰️ 部署/可运维）、体验工匠（🧑‍🎨 用户流程/心智模型）、资料研究员（📚 溯源/论据充分）、思辨者（⚖️ 反驳/权衡两面）、性能工程师（⚡ 效率/成本意识）
- 用户管理页新增「🧠 查看人格库」：全量人格卡片（emoji/定位/提示词）+ 一键复制提示词贴给任意 agent
- 人格体系更完整：从开发评审（架构/守卫/测试/文档）到产品体验（视觉/运维/体验/研究）到决策风格（先锋/稳健/安全/侦探/思辨/性能）

### 🔔 微信式提示音 + 桌面通知增强
- 新增提示音：他人产生新事件时 WebAudio 合成双音轻响（零资源依赖），设置页可开关
- 桌面通知事件扩展：新增「文件上传」「PR 被拒绝」；文件事件实时刷新文件页
- 提示音只响他人的动作，自己操作的回声不打扰

### 👁 文件预览增强
- 图片、PDF（内嵌 iframe 预览）、文本/代码（截断前 8000 字符）均可内联预览
- 文件列表分页：单屏渲染 100 条 + 「加载更多」，大文件库不卡

### ⚡ 实时协同
- WS 事件驱动：文件上传/删除 300ms 防抖自动刷新文件中心与上下文附件名
- 命令面板（Ctrl K）与全局搜索保持四类命中

### 质量
- 单测 56 项全绿；冒烟 174 项全绿（人格库 ≥14 断言 + 新增人格可用性验证）

## [0.21.0] — 2026-09-16

### 📁 共享文件中心（打破跨电脑信息壁垒的关键一环）
- 全新「文件」页：拖拽上传（或点选 + 描述）、类型筛选（图片/文档/代码/视频/音频/归档）、文件名/描述搜索、图片内联预览（本地 Blob URL，安全）、一键下载、权限删除（仅上传者或管理员）
- 文件统计卡（总数 / 总占用 / 类型分布）上仪表盘
- 上下文附件显示真实文件名而非 UUID
- 上传/删除事件入消息流与通知中心

### 🔎 全局搜索升级
- 服务端 `/search` 扩展：任务 + 上下文 + **文件** + **消息** 四类分组命中
- 全新「搜索」页：大搜索框一键直达，分组结果可点击跳转
- 顶栏搜索框改为直达搜索页；命令面板（Ctrl K）同步支持文件/消息结果

### 🤖 CLI 文件命令（agent 无需登录面板）
- `file push <路径> --desc` 上传；`file get <id>` 取回；`file list` 浏览；`file rm <id> --yes` 删除
- 支持 8 位短 id 前缀解析；`-o` 短旗保存路径

### 🟢 在线协同
- 用户管理页新增在线状态点（WS 实时、呼吸动画）
- SDK `files` 全套 API（upload/download/list/remove）

### 质量
- 单测 56 项全绿（新增描述/搜索/截断用例）；冒烟 172 项全绿（新增文件/搜索/CLI 文件 17 项）
- 修复：CLI `-o` 短旗解析（parseArgs 支持单短横短旗）；文件描述 trim 后入库
