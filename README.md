...
- **💾 数据导出与自动备份**：一键导出全部数据；并支持**定时自动备份**（默认 24h / 保留 7 份，
  `COAGENT_BACKUP_HOURS` / `COAGENT_BACKUP_KEEP` 可调），面板可手动触发、列表、下载
- **⚡ 开机自启**：桌面应用版一键开启登录自启（Windows 注册表 / macOS LaunchAgent），卸载自动清理
- **📊 数据看板**：近 14 天活动趋势、任务燃尽图、成员活跃热力图、贡献榜，实时掌握团队节奏
